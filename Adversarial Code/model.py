...
class DomainQA(nn.Module):
    def __init__(self, bert_name_or_config, num_classes=6, hidden_size=768,
        ...
        # 决定是否同时使用cls和sep的embedding
        if concat:
            input_size = 2 * hidden_size
        else:
            input_size = hidden_size
        # 判别器: (linear -> relu -> dropout)*3 -> linear -> softmax
    ...
    # 这里就是训练标准的bertQA,只不过会跑一遍判别器搞到loss，用于反向传播
    def forward_qa(self, input_ids, token_type_ids, attention_mask, start_positions, end_positions, global_step):
        ...
        # 下面一大串讲的就是搞到bert的输出，然后接个softmax计算loss就完事了。
        # 有些乱七八糟的部分是用来处理答案不在当前文章范围内的情况
        logits = self.qa_outputs(sequence_output)
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)
        end_logits = end_logits.squeeze(-1)
    ...
    # 这里就是训练判别器了，搞到cls的embedding，算出loss完事
